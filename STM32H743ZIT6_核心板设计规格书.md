...
| EN | 悬空默认使能（低压差应用可加 UVLO 滞回，首版从简） |
| SS | **必须外接 CSS=10nF 到 GND**（慢启动 ≈4ms；TI 明示内部无软启动，CSS ≤27nF，慢启动须 1-10ms） |
...
- DCDC SS 引脚外接 CSS=10nF 到 GND（TI §7.3.5：内部无软启动；慢启动 ≈4ms，1-10ms 合规，CSS ≤27nF）。
...
| 规则 | 数值 |
|---|---|
| 最小线宽/间距 | 0.15mm / 0.15mm（6/6mil）；过孔 0.4mm 焊盘 / 0.2mm 孔（预布线及工艺下限，JLC 0.2/0.45） |
